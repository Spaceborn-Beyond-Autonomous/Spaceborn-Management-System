// src/components/Dashboard/CEO/Projects.jsx
import React, { useState, useEffect } from 'react';
import authService from '../../../services/authService';

const Projects = ({ userRole = 'CEO' }) => {
  ...
  const loadMockData = () => {
    ...
    if (selectedDepartment !== 'All') {
      filtered = mockProjects.filter(p => p.department === selectedDepartment);
    }
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      case 'at-risk': 
        return <span className="px-2 py-1 bg-red-100 text-red-800 rounded-full text-xs flex items-center gap-1">⚠️ At Risk</span>;
      case 'delayed': 
        return <span className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs flex items-center gap-1">⏰ Delayed</span>;
      case 'on-track': 
      case 'active': 
        return <span className="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs flex items-center gap-1">✓ On Track</span>;
      case 'completed': 
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">Manage and track all active projects</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        ...
        <div className="bg-yellow-50 rounded-lg p-4 border border-yellow-200 text-center">
          ...
          <p className="text-2xl font-bold text-yellow-700">{stats.delayed}</p>
        </div>
